...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeBar.createFilledBar(0xFF46699e, 0xFFc5daed);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "Get Out", 32);
		botplayTxt.setFormat(Paths.font("funkin.otf"), 60, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (ClientPrefs.hitSounds){
				if(!note.isSustainNote){
					FlxG.sound.play(Paths.sound(ClientPrefs.hitAudio));
				}
			}
			if (!note.isSustainNote)
			{
